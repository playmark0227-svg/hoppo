...
(function() {
  ...
  function renderAchievements() {
    ...
    grid.innerHTML = defs.map(def => {
      ...
      return `
        <div class="achieve-tile ${earned ? 'earned' : 'locked'}" data-id="${def.id}" title="${def.desc}">
          <div class="achieve-tile-icon">${earned ? def.icon : '🔒'}</div>
          <div class="achieve-tile-name">${earned ? def.name : '？？？'}</div>
          <div class="achieve-tile-desc">${earned ? def.desc : 'あんろっくじょうけん：あそんでみよう'}</div>
        </div>
      `;
    }).join('');
  }
...
#北方領土 #北方領土ステータス`;
  ...
  async function renderLicenseToBlob(s, rank, rankIdx, totalTiers) {
    ...
    // watermark map (faded)
    try {
      const mapImg = await loadImage('assets/images/hoppou/four-islands-map.png');
      ctx.save();
  ...
  function bind() {
    document.getElementById('profileSaveBtn').addEventListener('click', () => {
      ...
      Store.setName(name);
      UI.toast('なまえをほぞんしたよ', 'good');
      UI.refreshHeader();
      UI.refreshHome();
      render();
    });
    document.getElementById('resetBtn').addEventListener('click', () => {
      if (!confirm('すべてのデータをリセットしますか？この操作は元にもどせません。')) return;
      Store.reset();
